...
export async function get_links(page: string): Promise<Array<string>> {
    ...
    try {
        const link = "https://en.wikipedia.org/w/api.php?action=query"
        + "&format=json&prop=links&pllimit=500&plnamespace=0&origin=*"
        + "&titles=" + page;
        const request = await fetch(link);
        const data = await request.json();

        const process: Array<{ns: number, title: string}>
            = data.query.pages[Object.keys(data.query.pages)[0]].links;
...
export async function get_links_to(page: string, n: number): Promise<ProbingHashtable<string, true>> {
    ...
    try {
        const link = "https://en.wikipedia.org/w/api.php?action=query"
        + "&format=json&prop=linkshere&lhlimit=max&lhnamespace=0"
        + "&origin=*&titles=" + page;
        const request = await fetch(link);
        let current = await request.json();
        let arr: Array<{ns: number, title: string}>
            = current.query.pages[Object.keys(
                current.query.pages
            )[0]].linkshere;
        if(arr.length === 0) {
            return result;
        ...
        while(continue_string === "||") {
            ...
            const new_link = link + "&lhcontinue="
                + current.continue.lhcontinue;
            const new_request = await fetch(new_link);
            current = await new_request.json();
            arr = current.query.pages[Object.keys(
                current.query.pages
            )[0]].linkshere;
            arr.forEach(x => ph_insert(result, x.title, true));
...
export async function is_valid_page(page: string): Promise<boolean> {
    try {
        const link = "https://en.wikipedia.org/w/api.php?action=query"
        + "&format=json&prop=links&pllimit=1&plnamespace=0&origin=*"
        + "&titles=" + page;
        const request = await fetch(link);
        const data = await request.json();
        return data.query.pages[Object.keys(
            data.query.pages
        )[0]].links.length >= 1;
    } catch {
        // Promise rejected
